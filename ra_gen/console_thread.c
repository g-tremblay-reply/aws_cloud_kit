...
const flash_cfg_t user_flash_cfg = { .data_flash_bgo = false, .p_callback =
		Console_FlashCallback, .p_context = NULL,
...
const flash_instance_t user_flash = { .p_ctrl = &user_flash_ctrl, .p_cfg =
		&user_flash_cfg, .p_api = &g_flash_on_flash_hp };
...
const sci_uart_extended_cfg_t g_console_uart_cfg_extend = { .clock =
		...
		.noise_cancel = SCI_UART_NOISE_CANCELLATION_DISABLE, .rx_fifo_trigger =
				...
				&g_console_uart_baud_setting, .flow_control =
...
const uart_cfg_t g_console_uart_cfg = { .channel = 5, .data_bits =
		UART_DATA_BITS_8, .parity = UART_PARITY_OFF, .stop_bits =
		UART_STOP_BITS_1, .p_callback = Console_UartCallback, .p_context = NULL,
...
/* Instance structure to use this module. */
const uart_instance_t g_console_uart = { .p_ctrl = &g_console_uart_ctrl,
		.p_cfg = &g_console_uart_cfg, .p_api = &g_uart_on_sci };
extern uint32_t g_fsp_common_thread_count;

const rm_freertos_port_parameters_t console_thread_parameters = { .p_context =
		(void*) NULL, };
