...
{
	/*copies all data from the SensorIaqRawData buffers to temporary buffer
...
void hpf_window_peak_detect(struct spo2 *_sp)
{
	...
	last_pt = _sp->data_ptr + (uint16_t)ARRAY_LENGTH - (uint16_t)INTERVAL; // most recent point in SensorIaqRawData
	if (last_pt >= (uint16_t)ARRAY_LENGTH)
		last_pt -= (uint16_t)ARRAY_LENGTH;
	first_pt = _sp->data_ptr + (uint16_t)ARRAY_LENGTH - (uint16_t)INTERVAL - _sp->baseline_cnt - 1; // oldest point in SensorIaqRawData
